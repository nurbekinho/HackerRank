package net.nurbek.HackerRank.Algorithms.Implementation;

import java.util.ArrayList;
import java.util.List;

public class P64_LarrysArray
{
    public static void main(String[] args)
    {
        System.out.println(larrysArray(new int[]{1, 6, 5, 2, 4, 3}));
        System.out.println(larrysArray(new int[]{1, 2, 3, 6, 4, 5}));
        System.out.println(larrysArray(new int[]{1, 2, 3, 5, 4}));
        //System.out.println(larrysArray(new int[]{-2, -3, -5, -2, -1, -1, -1, 1, -3}));
    }

    static String larrysArray(int[] A)
    {
        List<Integer> numbers = new ArrayList();

        for(int a : A) numbers.add(a);

        for(int i = 0; i < numbers.size(); i++)
        {
            if (numbers.get(i) != i + 1)
            {
                for(int j = i; j < numbers.size(); j++)
                {
                    if(numbers.get(j) == i + 1)
                    {
                        numbers.remove(j);
                        numbers.add(i, i + 1);

                        if((j - i) % 2 != 0)
                        {
                            if(i + 2 >= numbers.size()) return "NO";

                            int temp = numbers.get(i + 1);
                            numbers.remove(i + 1);
                            numbers.add(i + 2, temp);
                        }
                    }
                }
            }
        }

        return "YES";
    }
}

// SOLVED //

/*
Larry has been given a permutation of a sequence of natural numbers incrementing from 1 as an array. He must determine
whether the array can be sorted using the following operation any number of times:
* Choose any 3 consecutive indices and rotate their elements in such a way that ABC -> BCA -> CAB -> ABC.

For example, if A = {1, 6, 5, 2, 4, 3}:
A		        rotate
[1,6,5,2,4,3]	[6,5,2]
[1,5,2,6,4,3]	[5,2,6]
[1,2,6,5,4,3]	[5,4,3]
[1,2,6,3,5,4]	[6,3,5]
[1,2,3,5,6,4]	[5,6,4]
[1,2,3,4,5,6]

YES

On a new line for each test case, print YES if  can be fully sorted. Otherwise, print NO.

Input Format
The first line contains an integer "t", the number of test cases.

The next "t" pairs of lines are as follows:
* The first line contains an integer "n", the length of "A".
* The next line contains "n" space-separated integers "A[i]".

Output Format
For each test case, print YES if  can be fully sorted. Otherwise, print NO.

Sample Input
3
3
3 1 2
4
1 3 4 2
5
1 2 3 5 4

Sample Output
YES
YES
NO

Explanation
In the explanation below, the subscript of A denotes the number of operations performed.

Test Case 0:
A[0] = {3, 1, 2} -> rotate(3, 1, 2) -> A[1] = {1, 2, 3}
A is now sorted, so we print "YES" on a new line.

Test Case 1:
A[0] = {1, 3, 4, 2} -> rotate(3, 4, 2) -> A[1] = {1, 4, 2, 3}
A[1] = {1, 4, 2, 3} -> rotate(4, 2, 3) -> A[2] = {1, 2, 3, 4}
A is now sorted, so we print "YES" on a new line.

Test Case 2:
No sequence of rotations will result in a sorted A. Thus, we print "NO" on a new line.
*/