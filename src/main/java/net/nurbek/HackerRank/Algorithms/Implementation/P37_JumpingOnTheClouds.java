...
public class P37_JumpingOnTheClouds
{
    ...
    static int jumpingOnClouds(int[] c)
    {
        ...
        while(index < c.length)
        {
            if(index + 1 >= c.length)
                break;
            else if(index + 2 < c.length && c[index + 2] == 0)
                index += 2;
            else
                index++;
...
others are cumulus. She can jump on any cumulus cloud having a number that is equal to the number of the current cloud
plus 1 or 2. She must avoid the thunderheads. Determine the minimum number of jumps it will take Emma to jump from her
...
clouds at indexes 1 and 5. She could follow the following two paths: 0->2->4->6 or 0->2->3->4->6. The first path takes
